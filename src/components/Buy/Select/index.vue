    ...
    methods:{
      ...
      buySeat: function(){
        ...
        for(let i=0;i<this.seatRow;i++){
        	for(let j=0;j<this.seatCol;j++){
        		if(oldArray[i][j]===1){
              self.seatHang.push(i+1);
              self.seatLie.push(j);
              oldArray[i][j]=2;
              console.log(self.seatHang);
              console.log(j);
            }
          }
        }
        this.seatArray = oldArray;
        console.log(self.seatHang);
        console.log(self.seatLie);
        //将已选择的座位放入要传往下一路由的参数中
        self.seatPay.seatLie = self.seatLie;
        self.seatPay.seatHang = self.seatHang;
      },
			//处理座位选择逻辑
      ...
      pay(){
        var self = this;
        this.$router.push({ name :'routerPay',params :{seatPay: self.seatPay}})
      }
    },
